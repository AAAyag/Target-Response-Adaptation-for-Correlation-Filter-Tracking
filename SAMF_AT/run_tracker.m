...
function [precision, overlap, fps, fn] = run_tracker(dataset, video, kernel_type, feature_type, show_visualization, show_plots,...
    padding,lambda, lambda2, output_sigma_factor,interp_factor,kernel_sigma,cell_size,hog_orientations,scale_step,number_scales)
...
if nargin < 4, feature_type = 'hogcolor'; end
if nargin < 5, show_visualization = ~strcmp(video, 'all'); end
if nargin < 6, show_plots = ~strcmp(video, 'all'); end
if nargin < 7, padding = 2; end %extra area surrounding the target
...
switch video
    ...
    case 'choose',
        ...
        if ~isempty(video),
            [precision, overlap, fps, fn] = run_tracker(dataset, video, kernel_type, feature_type, show_visualization, show_plots,...
                padding,lambda, lambda2, output_sigma_factor,interp_factor,kernel_sigma,cell_size,features.hog_orientations,scale_step,number_scales);
        end
    ...
    case 'all',
        ...
        for k = 1:numel(videos),
            [all_precisions(k), all_overlaps(k), all_fps(k), all_fn(k)] = run_tracker(dataset, videos{k}, kernel_type, feature_type, show_visualization, show_plots,...
                ...
                kernel_sigma,cell_size,features.hog_orientations,scale_step,number_scales);
        end
        ...
        result_filename = fullfile(sprintf(['%.3f_' '%.3f_' '%.3g' 'fps_' dataset '_' tracker_name '_' kernel_type '_' feature_type '_padding%.3g' '_lambda%.1e' '_lambda2%.1e' '_interp%.2g' 'cell%u' 'hog%u' 'Scstep%u' 'NumSC%u''.txt'],...
            mean_precision_otb,mean_overlap_otb,mean_fps_otb,padding,lambda,lambda2,interp_factor,cell_size,features.hog_orientations,scale_step,number_scales));
    ...
    otherwise
        ...
        [positions, rects, time] = tracker(video_path, img_files, pos, target_sz,padding, kernel, lambda,lambda2,trans_samples_2D, output_sigma_factor, interp_factor, ...
